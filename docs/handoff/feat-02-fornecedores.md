...
| T4 remoção do objeto | `src/server/services/compras/fornecedor.service.ts` | `removerDocumento()` deriva a key (`storageKey ?? urlRefParaKey(url)`) e chama `getObjectStorage().delete(key)` (best-effort, `try/catch` + `logger.warn`) antes de apagar o metadado — **só se `key.startsWith(prefixoTenant(ctx.tenantId))`** (isolamento B2). |
| B2 isolamento multi-tenant | `src/server/services/compras/fornecedor.service.ts` | `storageKey`/`url` chegam crus do cliente. `adicionarDocumento` deriva a key candidata e **rejeita** (`BusinessRuleError('STORAGE_KEY_CROSS_TENANT')`) se não pertencer ao prefixo do tenant → impede metadados envenenados na BD (fecha fuga no download). `removerDocumento` só apaga o objeto se a key for do tenant → nunca apaga objeto estrangeiro. Defesa em profundidade (escrita + remoção). |
| M1 teste de segurança | `.../compras/__tests__/fornecedor-documento-seguranca.test.ts` | 5 testes (Prisma+storage mockados, `prefixoTenant` real): registo cross-tenant rejeitado (via storageKey e via url-ref), registo próprio aceite; remoção cross-tenant não apaga objeto (mas remove metadado), remoção própria apaga. |
