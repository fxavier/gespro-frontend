...
| Spec | Estado | Migração | Notas de gate |
|---|---|---|---|
| 01 doc-core — storage S3 | ✅ mergido | `2000` (`storageKey/contentType/tamanhoBytes` nos 4 modelos de documento) | REJEITADO (BLOCKER: download assinava GET de key de origem-cliente sem reafirmar o prefixo do tenant → leitura cross-tenant; MAJOR: download sem permissão por recurso) → corrigido (asserção `key.startsWith(prefixoTenant(ctx.tenantId))` + `PERMISSAO_ESCRITA_POR_RECURSO`) → APROVADO. `derivarKey` property-tested; presign `withApi` (allowlist+10MB), adapter `local` p/ dev/CI, Terraform (bucket privado+SSE+CORS+IAM mínima) |
| 04 stock — entrada/saída/transferência | ✅ mergido | — | APROVADO (só NITs). Ambos os formulários "nova movimentação" eram mocks (`setTimeout`+`console.log`) → ligados às actions transaccionais reais; rota duplicada `/stock/movimentacao` consolidada na canónica |
| 05 rh-formacoes | ✅ mergido | — | APROVADO. Rotas `nova`+`[id]` (a lista já lá ligava, rotas não existiam); MAJOR de estados de participante por mapear no `StatusBadge` + NIT Enter-to-submit corrigidos |
| 03 ativos — categorias + docs | ✅ mergido | — | REJEITADO (BLOCKER cross-tenant no delete de objeto de storage via key forjada) → corrigido (asserção de prefixo na escrita+delete) → APROVADO. Rota `categorias/[id]/editar` (corrige 404); fechou o gap de delete de órfãos S3 |
| 02 fornecedores — contactos + docs | ✅ mergido | — | REJEITADO (mesmo BLOCKER cross-tenant no delete + falta de teste) → corrigido + teste de segurança → APROVADO. **Causa-raiz dos "contactos 0"**: `CreateFornecedorSchema` não tinha o campo `contactos` → Zod descartava-o silenciosamente; corrigido no schema (não no serviço, que já fazia o nested create) |
| 06 transporte — de-mock + rotas + KPIs | ✅ mergido | — | APROVADO (MAJOR = dívida de módulo: forms em `FormData`). De-mock página-a-página, 10 rotas nova/editar, KPIs reais, comandos (prova de entrega/manutenção/checklist), docs viatura/motorista. Corrigiu enum de combustível errado que o mock escondia (`GASOLEO/GPL`→`GASOLINA/DIESEL/ETANOL/GNV`). **tsc partiu na integração** (FormData `string`→enum das actions) → corrigido com Zod `safeParse` (narra o tipo e valida no cliente) |

**Vetor de segurança recorrente (apanhado em 3 workstreams pelo `code-reviewer`):** a `meta.key` do presign é derivada server-side mas volta ao cliente e é reenviada nas actions de registo (`url: z.string().url()` aceita `gestpro-storage:tenant/OUTRO/...`); sem asserção de prefixo, o `delete`/`presignGet` operavam sobre a key → destruição/leitura cross-tenant. **Correção-raiz na escrita** (rejeitar key sem prefixo do tenant no registo) + defesa em profundidade no delete/download. Fechado em doc-core, ativos e fornecedores.

**Follow-ups documentados (handoffs `feat-0{1..6}-*.md`):** limite hard de 10MB no PUT S3 (POST-policy), recurso `colaborador` sem modelo/action de documento, `terraform apply` real não corrido (sem AWS no ambiente), migrar forms de transporte para RHF+zodResolver (dívida de módulo), otimização de rota/portal do motorista/eventos assíncronos (fora de âmbito, spec 06), validade opcional de documento (exigiria migração), discrepância de versão de TS no monorepo (raiz resolve 5.9.3 e mostra 2 erros pré-existentes em rotas de PDF; erp pinado em 5.8.3, que o `pnpm check` usa — alinhar).
