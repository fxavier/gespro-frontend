...
- [ ] **7. UI `/tesouraria`**
  ...
  - [x] 7.0 **ARBITRADO (orquestrador)** — o literal `'/tesouraria/compromissos/[id]'` **sai** das actions. O `createSafeAction` chama `revalidatePath(p)` sem `type` (`safe-action.ts:114`) e o Next só casa segmento dinâmico com `type: 'page'`, logo o literal nunca revalidou nada. **Não** se altera o `createSafeAction`: o golden standard (`compras.actions.ts:45,65`) nunca declara caminho dinâmico — revalida o caminho **concreto** da listagem mais uma **tag**, e a página de detalhe re-renderiza por ser Server Component sem `use cache`. Segue-se o mesmo: `paths: ['/tesouraria', '/tesouraria/compromissos']` + `tags: ['financas:tesouraria']` nas três mutações. Se um dia a página de detalhe optar por cache, declara `cacheTag('financas:tesouraria')` e o `updateTag` — que o `createSafeAction` **já** chama — cobre-a. A armadilha da API fica na [issue #68](https://github.com/fxavier/gespro-frontend/issues/68)
