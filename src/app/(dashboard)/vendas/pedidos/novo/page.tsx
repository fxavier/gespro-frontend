...
export default function NovoPedidoPage() {
	...
	const removerItem = (itemId: string) => {
		...
		if (item) {
			setValidacoesStock(
				validacoesStock.filter((v) => v.produtoId !== item.produtoId)
			);
		}
	};
			...
			itens.map((item) => {
				if (item.id === itemId) {
					...
					if (!disponivel && produto) {
						...
						setValidacoesStock((prev) =>
							prev.filter((v) => v.produtoId !== item.produtoId)
						);
					}
												...
												{produtos.map((produto) => (
													<SelectItem key={produto.id} value={produto.id}>
														<div className='flex items-center justify-between w-full'>
															...
															<Badge
																variant={
																	produto.stock > 0 ? 'default' : 'destructive'
																}
																className='ml-2'
															>
																Stock: {produto.stock}
			...
			<AlertDialog open={showStockAlert} onOpenChange={setShowStockAlert}>
				<AlertDialogContent>
					<AlertDialogHeader>
						...
						<AlertDialogDescription>
							Alguns produtos não têm stock suficiente. Deseja continuar mesmo
							assim? O pedido será criado mas precisará aguardar reposição de
							stock.
						</AlertDialogDescription>
					</AlertDialogHeader>
					<AlertDialogFooter>
