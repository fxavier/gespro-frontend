...
/**
 * Formulário de emissão de fatura — CLIENT COMPONENT.
...
export function NovaFaturaForm({ series, clientes }: NovaFaturaFormProps) {
    ...
    if (!state.ok) {
      ...
      if (details?.fieldErrors) {
        Object.entries(details.fieldErrors).forEach(([field, messages]) => {
          form.setError(field as keyof EmitirFaturaInput, { type: 'server', message: messages[0] });
        });
      } else {
        toast.error(state.error.message ?? 'Ocorreu um erro ao emitir a fatura.');
      }
    } else {
  ...
  const handleCancel = () => {
    if (isDirty) {
      const ok = window.confirm('Tem alterações não guardadas. Pretende mesmo sair?');
      if (!ok) return;
    }
    router.push('/vendas/faturas');
  };
  ...
  return (
    <Form {...form}>
      ...
      >
        {/* Dados gerais */}
        <FormSection title="Dados da Fatura" description="Informações gerais do documento fiscal">
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <Select onValueChange={field.onChange} defaultValue="MZN">
                    <FormControl>
                      <SelectTrigger>
                        <SelectValue />
                      </SelectTrigger>
        ...
        <FormSection
          ...
          description="Produtos ou serviços a faturar"
        >
              ...
              <div key={field.id} className="border rounded-lg p-4 space-y-3">
                ...
                <FormField
                  ...
                  render={({ field: f }) => (
                    <FormItem>
                      ...
                      <FormControl><Input placeholder="Descrição do produto/serviço" {...f} /></FormControl>
                      <FormMessage />
                    </FormItem>
                  )}
                />
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                  ...
                  <FormField
                    ...
                    render={({ field: f }) => (
                      <FormItem>
                        <FormLabel className="text-xs">Preço Unit. (MT) *</FormLabel>
                        <FormControl>
                    ...
                    render={({ field: f }) => (
                      <FormItem>
                        ...
                        >
                          ...
                          <SelectContent>
                            <SelectItem value="0.16">16% (padrão)</SelectItem>
