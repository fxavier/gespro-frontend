...
export default function NotaCreditoPage() {
  ...
  const handleSelecionarFatura = (fatura: Fatura) => {
    ...
    setItens(fatura.itens.map(item => ({
      ...item,
      selecionado: false,
      quantidadeCredito: 0
    })));
  ...
  const handleAtualizarQuantidade = (id: string, quantidade: number) => {
    setItens(itens.map(item => 
      item.id === id 
        ? { ...item, quantidadeCredito: Math.min(quantidade, item.quantidade) }
        : item
    ));
  };

  const calcularTotais = () => {
    const itensSelecionados = itens.filter(i => i.selecionado && i.quantidadeCredito > 0);
    
    const subtotal = itensSelecionados.reduce((acc, item) => {
      const valorProporcional = (item.subtotal / item.quantidade) * item.quantidadeCredito;
      return acc + valorProporcional;
    }, 0);

    const ivaTotal = itensSelecionados.reduce((acc, item) => {
      const ivaProporcional = (item.ivaItem / item.quantidade) * item.quantidadeCredito;
      return acc + ivaProporcional;
    }, 0);

    const total = subtotal + ivaTotal;

    return { subtotal, ivaTotal, total, quantidadeItens: itensSelecionados.length };
  };
  ...
  const handleGerarNotaCredito = () => {
    ...
    const notaCredito = {
      ...
      itens: itensSelecionados.map(item => ({
        ...item,
        quantidade: item.quantidadeCredito
      })),
    ...
    <div className="container mx-auto p-6 space-y-6">
      <div className="flex justify-between items-center">
        ...
        <div className="flex gap-2">
          ...
          <Button onClick={handleGerarNotaCredito}>
            ...
            <CardContent>
              {faturaSelecionada ? (
                ...
                <Button 
                  variant="outline" 
                  className="w-full"
                  onClick={() => setDialogFaturaAberto(true)}
                >
            ...
            <CardContent className="space-y-4">
              ...
              <div className="pt-4 border-t space-y-2">
                <div className="flex justify-between text-sm">
                  ...
                  <span className="font-medium">{totais.quantidadeItens}</span>
                </div>
                <div className="flex justify-between text-sm">
                  ...
                  <span className="font-medium">
                    {itens
                      ...
                      .reduce((acc, item) => acc + item.quantidadeCredito, 0)}
                  </span>
                </div>
              </div>
                ...
                <div className="pt-4 border-t">
                  ...
                  <div className="bg-muted p-3 rounded-lg space-y-1">
                    ...
                    <p className="text-sm text-muted-foreground">
                      Total: {faturaSelecionada.total.toFixed(2)} MT
                    </p>
                    <p className="text-sm text-muted-foreground">
                      Data: {new Date(faturaSelecionada.dataEmissao).toLocaleDateString('pt-PT')}
                    </p>
                  </div>
                </div>
              )}
          ...
          <Card>
            ...
            <CardContent className="space-y-2">
              <Button 
                ...
                disabled={!faturaSelecionada || totais.quantidadeItens === 0 || !motivo}
              >
