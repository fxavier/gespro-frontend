...
import { Inbox, Eye, AlertCircle } from 'lucide-react';
...
export default function CaixaEntrada() {
  ...
  const loadTickets = () => {
    ...
    const ticketsNaoAtribuidos = allTickets.filter(
      t => !t.atribuidoParaId && ['aberto', 'em_progresso'].includes(t.status)
    );
    setTickets(ticketsNaoAtribuidos);
    setLoading(false);
  };
    ...
    <div className="container mx-auto p-6 space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">Tickets não atribuídos aguardando atendimento</p>
        </div>
        ...
        <CardContent className="p-0">
          {tickets.length === 0 ? (
            <div className="text-center py-12">
              ...
              <p className="text-muted-foreground">Nenhum ticket na caixa de entrada</p>
            </div>
          ) : (
