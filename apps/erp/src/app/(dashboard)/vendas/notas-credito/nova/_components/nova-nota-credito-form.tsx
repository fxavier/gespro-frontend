...
/**
 * Formulário de emissão de nota de crédito — CLIENT COMPONENT.
...
export function NovaNotaCreditoForm({ series, faturas }: NovaNotaCreditoFormProps) {
    ...
    if (!state.ok) {
      ...
      if (details?.fieldErrors) {
        Object.entries(details.fieldErrors).forEach(([field, messages]) => {
          form.setError(field as keyof EmitirNotaCreditoInput, { type: 'server', message: messages[0] });
        });
      } else {
        toast.error(state.error.message ?? 'Ocorreu um erro ao emitir a nota de crédito.');
      }
    } else {
      toast.success('Nota de crédito emitida com sucesso!');
      router.push('/vendas/notas-credito');
    }
  }, [state, form, router]);
  ...
  return (
    <Form {...form}>
      ...
      >
        <FormSection title="Dados da Nota de Crédito" description="Associe a fatura original e indique o motivo da emissão">
        ...
        <FormSection
          title="Itens a Creditar"
          description="Especifique os produtos ou serviços a creditar"
        >
                ...
                <FormField
                  ...
                  render={({ field: f }) => (
                    <FormItem>
                      ...
                      <FormControl><Input placeholder="Descrição do item a creditar" {...f} /></FormControl>
                      <FormMessage />
                    </FormItem>
                  )}
                />
